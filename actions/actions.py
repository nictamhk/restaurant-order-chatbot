...
class ValidateOrderForm (FormValidationAction):
    ...
    async def run(self, dispatcher, tracker, domain):
        ...
        if (user_main is not None and user_main not in MENU['main']):
            dispatcher.utter_message(template = "utter_item_not_offered")
            ...
            dispatcher.utter_message("Alright,", user_main, "for the main course.")
...
class CalculateSubTotal (Action):
    ...
    async def run(self, dispatcher, tracker, domain):
        ...
        if end_order:
            dispatcher.utter_message(template = "utter_total", subtotal = str(curr_subtotal))